...
static u64 sys_close(machine_t *m) {
    GET(a0, fd);
    if (fd > 2) return close(fd);  // aviod to close standard i/o stream
    return 0;
}
...
static u64 sys_write(machine_t *m) {
    ...
    GET(a1, buf);
    GET(a2, len);
    return write(fd, (void *)TO_HOST(buf), (size_t)len);
}
...
static u64 sys_gettimeofday(machine_t *m) {
    ...
    if (tv_addr != 0) tz = (struct timezone *)TO_HOST(tz_addr);
    return gettimeofday(tv, tz);
}

static u64 sys_brk(machine_t *m) {
    GET(a0, addr);
    if (addr == 0) addr = m->mmu.guest_alloc;
    assert(addr >= m->mmu.base);
    i64 incr = (i64)addr - m->mmu.base;
...
static u64 sys_read(machine_t *m) {
    ...
    GET(a1, buf);
    GET(a2, len);
    return read(fd, (void *)TO_HOST(buf), (size_t)len);
}
...
static u64 sys_openat(machine_t *m) {
    ...
    GET(a2, flags);
    GET(a3, mode);
    return openat(fd, (char *)TO_HOST(name), convert_flags(flags), mode);
}
...
static u64 sys_open(machine_t *m) {
    ...
    GET(a1, flags);
    GET(a2, mode);
    return open((char *)TO_HOST(name), convert_flags(flags), mode);
}
...
u64 do_syscall(machine_t *m, u64 n) {
    ...
    if (n < ARRAY_SIZE(syscall_table))
        f = syscall_table[n];
    else if (n - OLD_SYSCALL_THRESHOLD < ARRAY_SIZE(syscall_table))
        f = old_syscall_table[n - OLD_SYSCALL_THRESHOLD];

    if (!f) Fatal("unkown syscall!");
